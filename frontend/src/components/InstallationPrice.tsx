...
const InstallationFeesSection = () => {
  ...
  return (
    <section className="py-8 sm:py-14 md:py-28 bg-gray-900 relative overflow-hidden">
      {/* Animated Background Elements */}
      <div className="absolute inset-0">
        <div className="absolute top-0 left-0 w-48 h-48 sm:w-72 sm:h-72 bg-blue-500/5 rounded-full filter blur-2xl sm:blur-3xl animate-pulse"></div>
        <div className="absolute bottom-0 right-0 w-48 h-48 sm:w-72 sm:h-72 bg-purple-500/5 rounded-full filter blur-2xl sm:blur-3xl animate-pulse delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-green-500/5 rounded-full filter blur-2xl sm:blur-3xl animate-pulse delay-500"></div>
      </div>

      <div className="wrapper relative z-10 max-w-7xl mx-auto px-3 sm:px-4">
        {/* Header */}
        <motion.div
          initial="hidden"
          whileInView="visible"
        ...
        >
          <motion.h2
            variants={itemVariants}
            className="mb-3 font-bold text-center text-white text-xl sm:text-2xl md:text-4xl lg:text-5xl"
          >
            Professional Installation Services
          </motion.h2>
          <motion.p
            variants={itemVariants}
            className="max-w-2xl mx-auto leading-6 text-white/50 text-sm sm:text-base md:text-lg"
          >
            Get connected with our expert installation team. Choose the perfect solution 
            for your needs with transparent pricing and professional service.
          </motion.p>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            {installationPlans.map((plan) => (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => handleTabChange(plan.id)}
                className={`flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-3 md:px-6 md:py-4 rounded-xl sm:rounded-2xl font-medium transition-all duration-300 text-xs sm:text-sm md:text-base ${
                  selectedPlan === plan.id
                    ? `${colorMap[plan.color].solid} text-white shadow-lg sm:shadow-2xl`
                    : 'bg-gray-800 text-gray-300 hover:bg-gray-700'
                }`}
              >
                <div className={`text-sm sm:text-base ${selectedPlan === plan.id ? 'text-white' : colorMap[plan.color].text}`}>
                  {plan.icon}
                </div>
          ...
          <div className="grid lg:grid-cols-12 gap-4 sm:gap-6 lg:gap-8">
            ...
            >
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  exit={{ opacity: 0, x: 50 }}
                  transition={{ duration: 0.5 }}
                  className={`relative rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 border-2 ${colorMap[currentPlan.color].border} bg-gradient-to-br from-gray-800 to-gray-900 shadow-lg sm:shadow-2xl w-full overflow-hidden`}
                >
                  ...
                  <div className="absolute inset-0 overflow-hidden rounded-2xl sm:rounded-3xl">
                    <motion.div
                      ...
                      className="absolute left-2 sm:left-4 md:left-8 top-1/4 w-8 h-8 sm:w-12 sm:h-12 md:w-16 md:h-16 bg-white/10 rounded-full blur-lg sm:blur-xl"
                    />
                    <motion.div
                      animate={{
                        y: [0, 8, 0],
                      ...
                      className="absolute right-3 sm:right-6 md:right-12 top-1/2 w-6 h-6 sm:w-8 sm:h-8 md:w-12 md:h-12 bg-white/10 rounded-full blur-md sm:blur-lg"
                    />
                  </div>

                  {/* Plan Header - Stacked on mobile */}
                  <div className="relative z-10 flex flex-col gap-4 sm:gap-6 mb-6 sm:mb-8">
                    <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4">
                      <div className={`p-2 sm:p-3 md:p-4 rounded-xl sm:rounded-2xl ${colorMap[currentPlan.color].bg} self-center sm:self-start`}>
                        <div className={`text-lg sm:text-xl md:text-2xl ${colorMap[currentPlan.color].text}`}>
                          {currentPlan.icon}
                        </div>
                      </div>
                      <div className="flex-1 text-center sm:text-left">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-2">
                          ...
                          {currentPlan.popular && (
                            <span className="px-2 py-1 bg-orange-500 text-white text-xs sm:text-sm font-medium rounded-full self-center sm:self-auto">
                              Most Popular
                            </span>
                          )}
                        </div>
                  ...
                  <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
                    <div>
                      <h4 className="font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2 text-sm sm:text-base">
                        <FiCheck className={`w-3 h-3 sm:w-4 sm:h-4 ${colorMap[currentPlan.color].text}`} />
                        What's Included
                      </h4>
                      <ul className="space-y-2 sm:space-y-3">
                        {currentPlan.features.map((feature, index) => (
                          ...
                          >
                            <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${colorMap[currentPlan.color].solid} mt-1.5 flex-shrink-0`} />
                    ...
                    <div>
                      <h4 className="font-semibold text-white mb-3 sm:mb-4 flex items-center gap-2 text-sm sm:text-base">
                        <FiUsers className={`w-3 h-3 sm:w-4 sm:h-4 ${colorMap[currentPlan.color].text}`} />
                        Perfect For
                      </h4>
                      <div className="flex flex-wrap gap-1 sm:gap-2 mb-4 sm:mb-6">
                        {currentPlan.bestFor.map((useCase, index) => (
                          <motion.span
                            ...
                            animate={{ opacity: 1, scale: 1 }}
                            transition={{ delay: index * 0.1 }}
                            className={`px-2 py-1 rounded-full text-xs ${colorMap[currentPlan.color].bg} ${colorMap[currentPlan.color].text} border ${colorMap[currentPlan.color].border}`}
                          >
                            {useCase}
                          </motion.span>
                        ))}
                      ...
                      <div className="space-y-2 sm:space-y-3">
                        <div className="flex items-center gap-2 sm:gap-3 text-white/80 text-xs sm:text-sm">
                          <FiClock className={`w-3 h-3 sm:w-4 sm:h-4 ${colorMap[currentPlan.color].text}`} />
                          <span>Installation Time: <strong className="text-white">{currentPlan.installationTime}</strong></span>
                        </div>
                        <div className="flex items-center gap-2 sm:gap-3 text-white/80 text-xs sm:text-sm">
                          <FiTool className={`w-3 h-3 sm:w-4 sm:h-4 ${colorMap[currentPlan.color].text}`} />
                          <span>Professional equipment setup included</span>
                        </div>
                        <div className="flex items-center gap-2 sm:gap-3 text-white/80 text-xs sm:text-sm">
                          <FiShield className={`w-3 h-3 sm:w-4 sm:h-4 ${colorMap[currentPlan.color].text}`} />
                          <span>Quality guaranteed</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => currentPlan.id === 'starlink' ? setShowContactForm(true) : console.log('Schedule installation for:', currentPlan.name)}
                    className={`relative z-10 w-full ${colorMap[currentPlan.color].solid} text-white px-4 py-3 sm:px-6 sm:py-4 md:px-8 md:py-4 rounded-xl sm:rounded-2xl font-semibold text-sm sm:text-base md:text-lg hover:shadow-xl sm:hover:shadow-2xl transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3`}
                  >
                    {currentPlan.id === 'starlink' ? (
                      <>
                        <FiInfo className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5" />
                        <span className="whitespace-nowrap">Request a Call Back</span>
                        <FiArrowRight className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5" />
                      </>
                    ) : (
                      <>
                        <span className="whitespace-nowrap">Schedule Installation</span>
                        <FiArrowRight className="w-3 h-3 sm:w-4 sm:h-4 md:w-5 md:h-5" />
                      </>
                    )}
                  </motion.button>
            ...
            >
              <div className="space-y-4 sm:space-y-6">
                ...
                <motion.div
                  variants={itemVariants}
                  className="rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-700/50 bg-gradient-to-br from-gray-800 to-gray-900"
                >
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {[
                      ...
                      <div key={index} className="flex items-center gap-2 sm:gap-3 text-white/80 text-xs sm:text-sm">
                        <FiCheck className="w-3 h-3 sm:w-4 sm:h-4 text-green-400 flex-shrink-0" />
                        <span>{benefit}</span>
                      </div>
                    ))}
                  </div>
          ...
          <motion.div
            variants={itemVariants}
            className="bg-gradient-to-r from-green-500/10 to-blue-500/10 rounded-xl sm:rounded-2xl md:rounded-3xl p-4 sm:p-6 md:p-8 overflow-hidden border-2 border-green-500/20 mt-4 sm:mt-6 md:mt-8 relative"
          >
            {/* Background Pattern - Reduced on mobile */}
            <div className="absolute inset-0 opacity-10">
              <div className="absolute top-0 right-0 w-32 h-32 sm:w-48 sm:h-48 md:w-64 md:h-64 bg-green-500 rounded-full filter blur-xl sm:blur-2xl md:blur-3xl"></div>
              <div className="absolute bottom-0 left-0 w-32 h-32 sm:w-48 sm:h-48 md:w-64 md:h-64 bg-blue-500 rounded-full filter blur-xl sm:blur-2xl md:blur-3xl"></div>
            </div>

            <h3 className="font-bold text-white text-lg sm:text-xl md:text-2xl lg:text-4xl mb-2 sm:mb-3 md:mb-4 text-center relative z-10">Need Help Choosing?</h3>
            <p className="text-blue-100 mb-3 sm:mb-4 text-xs sm:text-sm md:text-base text-center relative z-10">
              Our experts can recommend the best installation type for your specific needs.
            </p>
            <button 
              onClick={() => setShowContactForm(true)}
              className="w-full bg-gradient-to-r from-orange-400 to-orange-600 text-white text-center px-4 py-3 sm:px-6 sm:py-4 rounded-xl sm:rounded-2xl font-semibold text-sm sm:text-base md:text-lg hover:shadow-xl sm:hover:shadow-2xl transition-all duration-300 flex items-center justify-center gap-2 sm:gap-3 relative z-10"
            >
        ...
        {showContactForm && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.8, opacity: 0 }}
              className="relative bg-gray-800 rounded-xl sm:rounded-2xl md:rounded-3xl max-w-md w-full p-4 sm:p-6 md:p-8 border-2 border-blue-500/20 mx-auto"
              onClick={(e) => e.stopPropagation()}
            >
              <button
                onClick={() => setShowContactForm(false)}
              ...
              <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-white mb-3 sm:mb-4 pr-8">
                Request Installation Quote
              </h3>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-3 sm:space-y-4">
                <input
                  type="text"
                  placeholder="Full Name"
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-gray-700 border border-gray-600 rounded-lg sm:rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 text-sm sm:text-base"
                  required
                />
                <input
                  type="tel"
                ...
                />
                <select 
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 bg-gray-700 border border-gray-600 rounded-lg sm:rounded-xl text-white focus:outline-none focus:border-blue-500 text-sm sm:text-base"
                  value={selectedPlan}
                  onChange={(e) => setSelectedPlan(e.target.value)}
                  required
                >
                ...
                <div className="flex gap-2 sm:gap-3 md:gap-4">
                  <button
                    type="button"
                    onClick={() => setShowContactForm(false)}
                    className="flex-1 px-3 sm:px-4 py-2 sm:py-3 text-white/70 border border-gray-600 rounded-lg sm:rounded-xl hover:bg-gray-700 transition-colors text-sm sm:text-base"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 bg-blue-500 text-white px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl font-semibold hover:bg-blue-600 transition-colors text-sm sm:text-base"
                  >
                    Request Call
                  </button>
                </div>
