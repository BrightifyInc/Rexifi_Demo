...
const BenefitsSection = () => {
  ...
  >
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={`relative flex flex-col justify-between rounded-xl lg:rounded-3xl p-4 sm:p-6 md:p-8 lg:p-12 h-full min-h-[400px] sm:min-h-[500px] lg:min-h-[600px] border-2 ${colorMap[currentBenefit.color].border} mx-auto w-full max-w-full overflow-hidden`}
      >
        {/* Background Gradient */}
        <div className={`absolute inset-0 rounded-xl lg:rounded-3xl bg-gradient-to-br ${colorMap[currentBenefit.color].gradient} opacity-50`}></div>
        
        {/* Floating Elements - Reduced on mobile */}
        <div className="absolute inset-0 overflow-hidden rounded-xl lg:rounded-3xl">
          <motion.div
            animate={{
              y: [0, -10, 0],
              rotate: [0, 5, 0]
            ...
            className="absolute left-2 sm:left-4 lg:left-8 top-1/4 w-8 h-8 sm:w-12 sm:h-12 lg:w-16 lg:h-16 bg-white/10 rounded-full blur-lg sm:blur-xl"
          />
          <motion.div
            animate={{
              y: [0, 8, 0],
            ...
            className="absolute right-3 sm:right-6 lg:right-12 top-1/2 w-6 h-6 sm:w-10 sm:h-10 lg:w-12 lg:h-12 bg-white/10 rounded-full blur-md sm:blur-lg"
          />
        </div>

        <div className="relative z-10 flex-1 flex flex-col">
          <div className="flex-1">
            <div className="max-w-full mb-4 sm:mb-6 lg:mb-8">
              <div className="flex flex-col items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                <div className={`p-2 sm:p-3 rounded-xl ${colorMap[currentBenefit.color].bg}`}>
                  <div className={`text-lg sm:text-xl ${colorMap[currentBenefit.color].text}`}>
                    {currentBenefit.icon}
                  </div>
                </div>
                <h3 className="font-bold text-white text-lg sm:text-xl md:text-2xl lg:text-4xl text-center">
                  {currentBenefit.title}
                </h3>
              </div>
              <p className="text-sm sm:text-base lg:text-lg text-white/70 leading-relaxed text-center">
                {currentBenefit.description}
              </p>
            </div>

            {/* Features List - Single column on very small screens */}
            <div className="grid grid-cols-1 gap-2 sm:gap-3 mb-4 sm:mb-6 lg:mb-8">
              {currentBenefit.features.map((feature, index) => (
                <motion.div
                  ...
                  className="flex items-start gap-2 sm:gap-3 text-white/80 text-xs sm:text-sm lg:text-base"
                >
                  <FiCheck className={`w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 flex-shrink-0 mt-0.5 ${colorMap[currentBenefit.color].text}`} />
            ...
            <div className="grid grid-cols-3 gap-2 lg:gap-4 mb-4 sm:mb-6 lg:mb-8">
              {Object.entries(currentBenefit.stats).map(([key, value], index) => (
                <motion.div
                  ...
                  className="text-center p-1 sm:p-2 lg:p-3 bg-white/5 rounded-lg sm:rounded-xl backdrop-blur-sm"
                >
                  <div className="text-sm sm:text-base lg:text-lg font-bold text-white mb-0.5 sm:mb-1">{value}</div>
                  <div className="text-[10px] xs:text-xs text-white/60 capitalize leading-tight">{key}</div>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className={`relative z-10 w-full ${colorMap[currentBenefit.color].solid} text-white px-3 py-2 sm:px-4 sm:py-3 lg:px-8 lg:py-4 rounded-lg sm:rounded-xl lg:rounded-2xl font-semibold text-xs sm:text-sm lg:text-lg transition-all duration-300 hover:shadow-2xl flex items-center justify-center gap-1 sm:gap-2 lg:gap-3 mt-auto`}
          >
            <FiPlay className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5" />
            <span className="whitespace-nowrap text-xs sm:text-sm">Experience {currentBenefit.title.split(' ')[0]}</span>
            <FiArrowRight className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5" />
          </motion.button>
        </div>
      </motion.div>
    </AnimatePresence>
  ...
  >
    <div className="space-y-3 lg:space-y-6 h-full">
      {benefits.map((benefit) => (
        <motion.div
          ...
          className={`group p-3 sm:p-4 lg:p-6 rounded-lg sm:rounded-xl lg:rounded-2xl bg-gradient-to-br from-gray-800 to-gray-900 border-2 cursor-pointer transition-all duration-300 w-full ${
            activeBenefit === benefits.findIndex(b => b.id === benefit.id)
              ? `${colorMap[benefit.color].border} shadow-md lg:shadow-2xl scale-105` 
              : 'border-gray-700 hover:border-gray-600'
          }`}
        >
          <div className="flex items-start gap-2 sm:gap-3 lg:gap-4">
            <div className={`p-1 sm:p-2 lg:p-3 rounded-lg sm:rounded-xl ${colorMap[benefit.color].bg} group-hover:scale-110 transition-transform duration-300 flex-shrink-0`}>
              <div className={`text-sm sm:text-base ${colorMap[benefit.color].text}`}>
                {benefit.icon}
              </div>
            </div>
            <div className="flex-1 min-w-0">
              <h4 className="font-semibold text-white text-sm sm:text-base lg:text-lg mb-1 lg:mb-2 line-clamp-2 leading-tight">
                {benefit.title}
              </h4>
              ...
              <div className="flex items-center gap-1 sm:gap-2 mt-1 sm:mt-2 lg:mt-3">
                <div className="flex">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <FiStar key={star} className="w-2 h-2 sm:w-3 sm:h-3 lg:w-4 lg:h-4 text-yellow-400 fill-yellow-400" />
                  ))}
                </div>
                <span className="text-white/40 text-xs sm:text-sm">4.9/5</span>
              </div>
            </div>
          </div>
        </motion.div>
  ...
  <motion.div
    variants={itemVariants}
    transition={{ duration: 0.6, ease: "easeOut" }}
    className="lg:col-span-12 w-full mt-3 lg:mt-0"
  >
    <div className="relative bg-gradient-to-r from-green-500/10 to-blue-500/10 rounded-xl lg:rounded-3xl p-4 sm:p-6 md:p-8 lg:p-12 overflow-hidden border-2 border-green-500/20 w-full">
      {/* Background Pattern - Reduced on mobile */}
      <div className="absolute inset-0 opacity-10">
        <div className="absolute top-0 right-0 w-20 h-20 sm:w-32 sm:h-32 lg:w-64 lg:h-64 bg-green-500 rounded-full filter blur-xl sm:blur-3xl"></div>
        <div className="absolute bottom-0 left-0 w-20 h-20 sm:w-32 sm:h-32 lg:w-64 lg:h-64 bg-blue-500 rounded-full filter blur-xl sm:blur-3xl"></div>
      </div>

      <div className="relative z-10 lg:flex lg:items-center lg:justify-between">
        <div className="max-w-full lg:max-w-md mb-4 sm:mb-6 lg:mb-0">
          <h3 className="font-bold text-white text-base sm:text-xl md:text-2xl lg:text-4xl mb-2 sm:mb-3 lg:mb-4 text-center lg:text-left leading-tight">
            Ready to Experience Premium Internet?
          </h3>
          <p className="text-white/70 text-xs sm:text-base lg:text-lg mb-3 sm:mb-4 lg:mb-6 text-center lg:text-left leading-relaxed">
            Join 50,000+ satisfied customers across Africa enjoying reliable, high-speed internet.
          </p>
          <motion.button
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="w-full bg-gradient-to-r from-orange-400 to-orange-600 text-white px-3 py-2 sm:px-4 sm:py-3 lg:px-8 lg:py-4 rounded-lg sm:rounded-xl lg:rounded-2xl font-semibold text-xs sm:text-sm lg:text-lg hover:shadow-2xl transition-all duration-300 flex items-center justify-center gap-1 sm:gap-2 lg:gap-3 mx-auto lg:mx-0"
          >
            <FiZap className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5" />
            <span className="whitespace-nowrap">Get Rexifi Today</span>
            <FiArrowRight className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5" />
          </motion.button>
        </div>
        
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 lg:gap-6 text-center">
          {stats.map((stat, index) => (
            <motion.div
              ...
              transition={{ delay: index * 0.2 }}
              className="text-white flex-1 min-w-[70px] xs:min-w-[80px] sm:min-w-[100px]"
            >
              <div className="flex flex-col items-center justify-center gap-0.5 sm:gap-1 lg:gap-2 mb-0.5 sm:mb-1 lg:mb-2">
                <div className="text-orange-400 text-sm sm:text-base">
                  {stat.icon}
                </div>
                <div className="text-base sm:text-xl lg:text-3xl font-bold text-orange-400">{stat.number}</div>
              </div>
              <div className="text-white/60 text-[10px] xs:text-xs sm:text-sm leading-tight">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
